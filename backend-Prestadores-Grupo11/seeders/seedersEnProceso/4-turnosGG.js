...
module.exports = {
  up: async (queryInterface, Sequelize) => {
    ...
    const ESPECIALIDADES_POR_PERFIL = {
      ...
      oncologico: ['Oncología', 'Hematología'],
      materno: ['Obstetricia', 'Ginecología'],
      pediatrico: ['Pediatría', 'Cardiología Infantil']
    };
    ...
    function generarNotesCompleto({ start, medico, centro, afiliadoTitular, integrantePaciente, pacienteData, especialidad, perfil, archivedAt, modifiedAt, esCesarea = false, cesareaTipo = 'Cesárea programada' }) {
      const edad = Math.floor(yearsBetween(pacienteData.fecha_nacimiento, HOY));
      const duracionSintoma = randomItem([`${randInt(1, 3)}`, `${randInt(4, 7)}`]);
      ...
      const nroAf = integrantePaciente ? `${afiliadoTitular.numero_afiliado}-${String(integrantePaciente.sufijo).padStart(2,'0')}` : `${afiliadoTitular.numero_afiliado}-${String(afiliadoTitular.sufijo).padStart(2,'0')}`;
      ...
      bloques.push(`Sexo: ${pacienteData.sexo}`);
      ...
      if(plantilla) {
        bloques.push(`<b>Motivo de consulta:</b> ${plantilla.motivo.join(' ')}`);
        bloques.push(`<b>Anamnesis:</b> ${plantilla.anamnesis.join(' ')}`);
        bloques.push(`<b>Examen físico:</b> ${plantilla.examenFisico.join(' ')}`);
        bloques.push(`<b>Diagnóstico:</b> ${plantilla.diagnostico.join(' ')}`);
        bloques.push(`<b>Plan:</b> ${plantilla.plan.join(' ')}`);
      }
    ...
    for(const a of afiliados) {
      ...
      if(a.sexo==='femenino' && edad>=18 && edad<=45 && Math.random()<0.06) p='materno';
      perfilPorPersona.set(`A-${a.id}`,p);
    }
    for(const i of integrantes){
      let p=elegirPerfil();
      const edad = Math.floor(yearsBetween(i.fecha_nacimiento,HOY));
      if(edad<18) p='pediatrico';
      if(i.sexo==='femenino' && edad>=18 && edad<=45 && Math.random()<0.06) p='materno';
      perfilPorPersona.set(`I-${i.id}`,p);
    }

    // ------------------- GENERAR TURNOS PASADOS Y FUTUROS -------------------
    ...
    for(const persona of todasPersonas){
      ...
      for(let t=0;t<totalTurnos;t++){
        ...
        const notes = generarNotesCompleto({start,medico:medicoSel,centro:centroSel,afiliadoTitular,integrantePaciente,pacienteData,especialidad:especialidadElegida,perfil,archivedAt,modifiedAt:modifiedAtTent,esCesarea});

        const contieneEvolucion = notes.includes('<b>Evolución:');
        const modifiedAtFinal = contieneEvolucion?modifiedAtTent:null;

        turnosToInsert.push({
          date: new Date(start.getTime()-randInt(1,5)*24*60*60*1000),
          start,
          duration,
          archivedAt,
    ...
    // ------------------- TURNOS FUTUROS -------------------
    // Se generan hasta NUM_DIAS_FUTUROS sin notes ni archived/modified
    // lógica igual que antes: slots por día, mañana/tarde, 50% afiliado/integrante
    // ...

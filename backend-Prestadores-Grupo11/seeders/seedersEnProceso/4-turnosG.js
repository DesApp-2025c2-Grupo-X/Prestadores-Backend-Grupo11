...
/**
 * Seeder definitivo de Turnos (versión final con reglas clínicas)
 *
 * - PEGAR TU objeto "casosPorEspecialidad" en la sección indicada abajo.
 * - Requiere tablas: Prestadors, Afiliados, Integrantes.
 * - Genera turnos pasados (con notes) respetando fecha_alta y perfiles,
 *   y turnos futuros hasta +NUM_DIAS_FUTUROS (sin notes).
 *
 * Reglas destacadas:
 * - 50% Afiliado / 50% Integrante
 * - N° Afiliado en notes: numero_afiliado-sufijo (si integrante: toma numero_afiliado del titular)
 * - Evolución: obligatoria para cronico/oncologico (si es turno pasado), probabilística para otros.
 * - Cesáreas: se generan notas de cesárea cuando corresponde (perfil materno).
 */
...
module.exports = {
  up: async (queryInterface, Sequelize) => {
    ...
    const DEFAULT_DAILY_RANGE = [6, 12]; // por médico por día (ajustable)
    // coeficientes turnos/año por perfil (realista)
    const PERFIL_COEF = {
      sano: [0, 1],
      agudo: [1, 3],
      cronico: [4, 10],
      oncologico: [10, 25],
      materno: [6, 20], // madres embarazadas generan más contactos
      pediatrico: [2, 10]
    };

    const ESPECIALIDADES_POR_PERFIL = {
      sano: ['Clínica Médica', 'Nutrición', 'Dermatología', 'Oftalmología'],
      agudo: ['Clínica Médica', 'Traumatología', 'Infectología', 'Gastroenterología', 'Neumonología'],
      cronico: ['Clínica Médica', 'Cardiología', 'Endocrinología', 'Nefrología', 'Neumonología'],
      oncologico: ['Oncología', 'Hematología', 'Diagnóstico por Imágenes', 'Cirugía General'],
      materno: ['Obstetricia', 'Ginecología', 'Diagnóstico por Imágenes', 'Neonatología'],
      pediatrico: ['Pediatría', 'Oncología Pediátrica', 'Cardiología Infantil']
    };
    ...
    // -------------- PEGÁ TU OBJETO DE PLANTILLAS AQUÍ --------------
    /*
      Ejemplo: casosPorEspecialidad = {
        "Endocrinología": [
          { motivo: [...], anamnesis: [...], examenFisico: [...], diagnostico: [...], plan: [...], evolucion: [...], antecedenteMedico: true },
          ...
        ],
        "Clínica Médica": [ ... ],
        ...
    ...
    */
    //const casosPorEspecialidad = {}; // <<--- REEMPLAZAR POR TU OBJETO
    ...
    if (!medicos.length) throw new Error('No hay medicos (role: "medico").');
    ...
    const distribucionPerfiles = [
      ...
      // materno y pediatrico los asignaremos en función de edad/sexo si aplican
    ];

    function elegirPerfilAleatorioGenerico() {
      const r = Math.random() * 100;
      let acc = 0;
      for (const d of distribucionPerfiles) {
        acc += d.porcentaje;
    ...
    for (const a of afiliados) {
      let perfil = elegirPerfilAleatorioGenerico();
      // si es mujer en edad fértil, pequeña probabilidad de materno
      const edad = Math.floor(yearsBetween(a.fecha_nacimiento, HOY));
      if (a.sexo === 'femenino' && edad >= 18 && edad <= 45 && Math.random() < 0.06) perfil = 'materno';
      // pediátrico no aplica a afiliado titular si mayor de 18
      perfilPorPersona.set(`A-${a.id}`, perfil);
    }
    // Integrantes
    for (const i of integrantes) {
      let perfil = elegirPerfilAleatorioGenerico();
      const edad = Math.floor(yearsBetween(i.fecha_nacimiento, HOY));
      if (edad < 18) perfil = 'pediatrico';
      if (i.sexo === 'femenino' && edad >= 18 && edad <= 45 && Math.random() < 0.06) {
        // posibilidad de ser madre integrante (raro)
        perfil = 'materno';
      }
      perfilPorPersona.set(`I-${i.id}`, perfil);
    }

    // -------------- VALIDACIONES POR ESPECIALIDAD --------------
    function calcularEdad(fecha_nacimiento) {
      return Math.floor(yearsBetween(fecha_nacimiento, HOY));
    }

    function especialidadValidaParaPaciente(especialidad, paciente) {
      if (!especialidad || !paciente) return false;
      const edad = calcularEdad(paciente.fecha_nacimiento);
      const esp = especialidad.toLowerCase();
      if (esp.includes('neonat')) return edad < 1;
      if (esp.includes('pediatr') || esp.includes('infantil')) return edad < 18;
      if (esp.includes('ginecolog') || esp.includes('obstetric')) return paciente.sexo === 'femenino';
      if (esp.includes('urolog')) return paciente.sexo === 'masculino';
      return true;
    }
  ...
  return text
    ...
    .replace(/\$\{antecedenteMedico\}/g, vars.antecedenteMedico || '');
}
    ...
    function generarAntecedenteMedicoGenerico(edad) {
      const opciones = [
        'Niega antecedentes patológicos de relevancia.',
        'Antecedentes: hipertensión arterial en tratamiento.',
        'Antecedentes familiares de enfermedad cardiovascular.',
        'Antecedentes quirúrgicos: apendicectomía años atrás.'
      ];
      if (edad >= 60) {
        return randomItem(['Antecedentes de hipertensión arterial.', 'Antecedentes de enfermedad coronaria en familia.']);
      }
      return randomItem(opciones);
    }

    function elegirPlantilla(especialidad) {
      const list = casosPorEspecialidad[especialidad];
      if (!Array.isArray(list) || list.length === 0) return null;
      return randomItem(list);
    }

    // Construye el N° Afiliado formateado
    function nroAfiliadoFormateado(afiliado, integrante) {
      if (integrante) {
        // integrante.sufijo existe; afiliado es el titular
        return `${afiliado.numero_afiliado}-${String(integrante.sufijo).padStart(2, '0')}`;
      } else {
        return `${afiliado.numero_afiliado}-${String(afiliado.sufijo).padStart(2, '0')}`;
      }
    }

    // Generar bloque de notes desde plantilla y reglas (incluye encabezado y cierre)
    function generarNotesCompleto({ start, medico, centro, afiliadoTitular, integrantePaciente, pacienteData, especialidad, perfil, archivedAt, modifiedAt, esCesarea = false, cesareaTipo = 'Cesárea programada' }) {
      const edad = calcularEdad(pacienteData.fecha_nacimiento);
      const duracionSintoma = randomItem([`${randInt(1, 3)} días`, `${randInt(4, 7)} días`, `${randInt(1, 4)} semanas`, `${randInt(1, 6)} meses`]);
      ...
      const nroAf = nroAfiliadoFormateado(afiliadoTitular, integrantePaciente);

      const bloques = [];

      bloques.push(`<b>Fecha:</b> ${fechaStr}`);
      bloques.push(`<b>Médico:</b> ${medicoStr} ${medico && medico.especialidades ? '– ' + medico.especialidades[0] : ''}`);
      bloques.push(`<b>Centro Médico:</b> ${centroStr}`);
      bloques.push('');
      bloques.push(`<b>Datos del paciente:</b>`);
      bloques.push(`Nombre: ${pacienteData.nombre} ${pacienteData.apellido}`);
      bloques.push(`Edad: ${edad} años`);
      bloques.push(`Sexo: ${pacienteData.sexo === 'femenino' ? 'Femenino' : 'Masculino'}`);
      bloques.push(`DNI: ${pacienteData.dni}`);
      bloques.push(`<b>N° Afiliado:</b> ${nroAf}`);
      bloques.push('');

      // Si es cesárea, generamos una nota especial reducida y coherente
      if (esCesarea) {
        bloques.push(`<b>Motivo de consulta:</b>`);
        bloques.push(cesareaTipo);
        bloques.push('');
        bloques.push(`<b>Anamnesis:</b>`);
        bloques.push('Embarazo a término. Riesgo obstétrico calculado. Paciente programada para cesárea electiva.');
        bloques.push('');
        bloques.push(`<b>Examen físico:</b>`);
        bloques.push('Signos vitales dentro de parámetros, abdomen grávido, actividad fetal presente.');
        bloques.push('');
        bloques.push(`<b>Diagnóstico:</b>`);
        bloques.push('Parto por cesárea programada por indicación obstétrica.');
        bloques.push('');
        bloques.push(`<b>Plan:</b>`);
        bloques.push('Internación, preparación quirúrgica, realizar cesárea según protocolo; derivar neonatología para evaluación inmediata.');
        bloques.push('');
        // evolución (si aplica)
        if (perfil === 'cronico' || perfil === 'oncologico' || true) {
          // para cesárea preferimos siempre generar evolución (post-operatorio)
          const evo = `Evolución post-operatoria: vigilancia hemodinámica y neonatal.`;
          bloques.push(`<b>Evolución:</b>`);
          bloques.push(evo);
        }
        // cierre con modifiedAt si existe
        if (modifiedAt) {
          bloques.push('');
          bloques.push(`<b>Última actualización:</b> ${modifiedAt.toISOString().slice(0, 16).replace('T', ' ')}`);
        }
        return bloques.join('\n\n');
      }

      // Plantilla "normal" por especialidad
      const plantilla = elegirPlantilla(especialidad);
      const vars = { edad, duracionSintoma, antecedenteMedico: '' };

      // manejo de antecedenteMedico: si la plantilla lo solicita e somos clinica medica, inyectar posible antecedente
      if (plantilla && plantilla.antecedenteMedico) {
        if (perfil === 'cronico' || perfil === 'oncologico') {
          vars.antecedenteMedico = generarAntecedenteMedicoGenerico(edad);
        } else {
          // rama NOT: no vacío sino texto genérico distinto por caso
          vars.antecedenteMedico = randomItem([
            'Niega antecedentes patológicos de interés.',
            'No refiere intervenciones quirúrgicas previas relevantes.',
            'Sin antecedentes familiares relevantes declarados.'
          ]);
        }
      }

      // Motivo / Anamnesis / Examen / Diagnóstico / Plan / Evolución (si corresponde)
      if (plantilla && plantilla.motivo) {
        bloques.push(`<b>Motivo de consulta:</b>`);
        bloques.push(plantillaArrayToText(plantilla.motivo, vars));
        bloques.push('');
      }

      if (plantilla && plantilla.anamnesis) {
        bloques.push(`<b>Anamnesis:</b>`);
        bloques.push(plantillaArrayToText(plantilla.anamnesis, vars));
        bloques.push('');
      }

      if (plantilla && plantilla.examenFisico) {
        bloques.push(`<b>Examen físico:</b>`);
        bloques.push(plantillaArrayToText(plantilla.examenFisico, vars));
        bloques.push('');
      }

      if (plantilla && plantilla.diagnostico) {
        bloques.push(`<b>Diagnóstico:</b>`);
        bloques.push(plantillaArrayToText(plantilla.diagnostico, vars));
        bloques.push('');
      }

      if (plantilla && plantilla.plan) {
        bloques.push(`<b>Plan:</b>`);
        bloques.push(plantillaArrayToText(plantilla.plan, vars));
        bloques.push('');
      }
      ...
      if (archivedAt && (requiereEvolucionSiempre || Math.random() < 0.4)) {
        tieneEvolucion = true;
      }

      if (tieneEvolucion) {
        // si plantilla tiene evoluciones posibles, usar una, sino generar una genérica
        bloques.push(`<b>Evolución:</b>`);
        if (plantilla && plantilla.evolucion && plantilla.evolucion.length) {
          bloques.push(randomItem(plantilla.evolucion));
        } else {
          bloques.push('Evolución favorable tras intervención. Seguimiento ambulatorio programado.');
        }
      }

      // Cierre con modifiedAt si hubo evolución
      if (tieneEvolucion && modifiedAt) {
        bloques.push('');
        bloques.push(`<b>Última actualización:</b> ${modifiedAt.toISOString().slice(0, 16).replace('T', ' ')}`);
      }
    ...
    const todasPersonas = [
      ...afiliados.map(a => ({ tipo: 'A', id: a.id, data: a })),
      ...integrantes.map(i => ({ tipo: 'I', id: i.id, data: i }))
    ];

    for (const persona of todasPersonas) {
      const clave = `${persona.tipo}-${persona.id}`;
      const perfil = perfilPorPersona.get(clave) || 'sano';

      // titular para referencia a fecha_alta
      let titular;
      if (persona.tipo === 'A') titular = persona.data;
      else titular = afiliadoById[persona.data.afiliadoId];
      ...
      const minPorAño = coefRange[0];
      const maxPorAño = coefRange[1];

      // total de turnos pasados aproximado
      let totalTurnos = 0;
      if (minPorAño === 0 && maxPorAño === 0) totalTurnos = 0;
      else totalTurnos = randInt(Math.floor(minPorAño * años), Math.ceil(maxPorAño * años));

      // limitar turnos por persona para evitar explosiones
      totalTurnos = clamp(totalTurnos, 0, 200);

      for (let t = 0; t < totalTurnos; t++) {
        // elegir especialidad preferente por perfil
        const posiblesEsp = ESPECIALIDADES_POR_PERFIL[perfil] || ['Clínica Médica'];
        let especialidadElegida = randomItem(posiblesEsp);
        // buscar medicos de esa especialidad
        let medList = medicos.filter(m => Array.isArray(m.especialidades) && m.especialidades[0] === especialidadElegida);

        // intenta encontrar médico compatible y especialidad válida para paciente
        let attempts = 0;
        while ((!medList || medList.length === 0 || !especialidadValidaParaPaciente(especialidadElegida, persona.data)) && attempts < 6) {
          especialidadElegida = randomItem(posiblesEsp.concat(Object.keys(casosPorEspecialidad || {})));
          medList = medicos.filter(m => Array.isArray(m.especialidades) && m.especialidades[0] === especialidadElegida);
          attempts++;
        }
        ...
        if (!medList || medList.length === 0) {
          medList = medicos;
          especialidadElegida = medList[0] && medList[0].especialidades ? medList[0].especialidades[0] : 'Clínica Médica';
        }

        const medicoSel = randomItem(medList);
        const centroSel = centros.find(c => c.id === medicoSel.centroId);

        // fecha aleatoria entre fechaAlta y ayer
        const fechaFinRange = addDays(HOY, -1);
        const fechaRand = faker.date.between({ from: fechaAlta, to: fechaFinRange });

        // hora razonable
        const hour = randInt(8, 17);
        const minute = [0, 15, 30, 45][randInt(0, 3)];
        const start = new Date(fechaRand);
        start.setHours(hour, minute, 0, 0);

        const duration = randomItem([15, 20, 30, 45]);
        const archivedAt = addMinutes(start, randInt(1, 30));
        // modifiedAt solo si hay evolucion (decidido en generarNotesCompleto)
        // definimos tentative modifiedAt en +0-14 días; lo validaremos según evolución
        ...
        // decidir si este turno es por cesárea (solo si perfil materno y especialidad obstetricia/gineco)
        let esCesarea = false;
        if (perfil === 'materno' && (Math.random() < 0.5)) {
          // decidimos generar un turno de cesárea en obstetricia
          especialidadElegida = 'Obstetricia';
          esCesarea = true;
        }

        // generar notes y decidir si hay evolucion allí
        // note: generarNotesCompleto requiere afiliado titular object y posible integrante object
        const afiliadoTitular = afiliadoById[afiliadoIdFinal];
        const integrantePaciente = integranteIdFinal ? integranteById[integranteIdFinal] : null;
        const pacienteData = persona.data;

        // decide modifiedAt real dependiendo si la nota tendrá evolución
        // generarNotesCompleto decidirá internamente si poner evolucion, pero necesitamos pasar modifiedAt o null
        // approach: pasar modifiedAtTent; dentro de generarNotesCompleto, si no hay evolucion, se ignorará
        const notes = generarNotesCompleto({
          start,
          medico: medicoSel,
          centro: centroSel,
          afiliadoTitular,
          ...
          perfil,
          archivedAt,
          modifiedAt: modifiedAtTent,
          esCesarea
        });

        // decide si la nota realmente contiene "Evolución" (buscamos la palabra "<b>Evolución:" en notes)
        const contieneEvolucion = notes.includes('<b>Evolución:');

        let modifiedAtFinal = contieneEvolucion ? modifiedAtTent : null;

        // asegurar ordering date/start/archivedAt/modifiedAt
        const dateField = new Date(start.getTime() - randInt(1, 5) * 24 * 60 * 60 * 1000); // date pedido del turno (dias antes)
        const turnoObj = {
          date: dateField,
          start,
          duration,
        ...
        turnosToInsert.push(turnoObj);

        if (turnosToInsert.length >= BATCH_SIZE) {
          await queryInterface.bulkInsert('Turnos', turnosToInsert.splice(0, turnosToInsert.length), {});
        }
      } // end for totalTurnos
    } // end for each persona

    // -------------- GENERAR TURNOS FUTUROS (HOY -> +NUM_DIAS_FUTUROS) --------------
    const fechaFinFuturo = addDays(HOY, NUM_DIAS_FUTUROS);

    for (const medico of medicos) {
      const especialidadMed = (Array.isArray(medico.especialidades) && medico.especialidades[0]) || 'Clínica Médica';
      // evitar crear turnos para Guardia/Terapia Intensiva
      const espLow = especialidadMed.toLowerCase();
      if (espLow.includes('guardia') || espLow.includes('terapia intensiva')) continue;
      ...
      for (let d = 0; d <= NUM_DIAS_FUTUROS; d++) {
        ...
        // decide n turnos para este medico este dia
        const nTurnosDia = randInt(dailyRange[0], dailyRange[1]);

        // generar slots en mañana/tarde
        ...
        const slots = [];
        const morningCount = Math.max(0, Math.floor(nTurnosDia * 0.6));
        const afternoonCount = Math.max(0, nTurnosDia - morningCount);

        let cursor = new Date(morningStart);
        for (let s = 0; s < morningCount; s++) {
          const duration = randomItem([15, 20, 30, 45]);
          const slotStart = new Date(cursor);
          slots.push({ start: slotStart, duration });
          cursor = addMinutes(cursor, duration + randInt(5, 10));
        }
        ...
        // si faltan slots, rellenar
        while (slots.length < nTurnosDia) {
          const hr = randInt(8, 18);
          const minute = [0, 15, 30, 45][randInt(0, 3)];
          const start = new Date(dia); start.setHours(hr, minute, 0, 0);
          slots.push({ start, duration: randomItem([15, 20, 30]) });
        }

        for (const slot of slots) {
          // elegir paciente 50/50 y validar especialidad
          ...
          // ensure paciente is compatible with specialty
          let tries = 0;
          while (!especialidadValidaParaPaciente(especialidadMed, pacienteObj) && tries < 6) {
          ...
          if (!especialidadValidaParaPaciente(especialidadMed, pacienteObj)) {
            // skip slot if cannot find compatible
            continue;
          }

          const afiliadoIdFinal = usarIntegrante ? pacienteObj.afiliadoId : pacienteObj.id;
          const integranteIdFinal = usarIntegrante ? pacienteObj.id : null;
          const titular = afiliadoById[afiliadoIdFinal];
          const fechaAltaTit = titular && titular.fecha_alta ? new Date(titular.fecha_alta) : addDays(HOY, -365);
          // if slot before fecha_alta, adjust to fecha_alta day/time
          if (slot.start < fechaAltaTit) {
            slot.start = new Date(fechaAltaTit);
            slot.start.setHours(8, 0, 0, 0);
          }
